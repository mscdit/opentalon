...
func newConcurrentOrch(t *testing.T, maxConcurrent int, llm LLMClient) (*Orchestrator, string, string) {
	...
	sessions.Create(state.SessionParams{ID: "s2"})
...
func (s *blockingSetSummaryStore) Create(p state.SessionParams) *state.Session {
	return s.inner.Create(p)
}
...
func (s *blockingSetSummaryStore) SetTitle(id, title string) error {
	return s.inner.SetTitle(id, title)
}
...
func TestSummarizeGoroutineHoldsSessionLock(t *testing.T) {
	...
	inner.Create(state.SessionParams{ID: sessionID})
...
func TestConcurrencySessionLockCleanup(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		sessions.Create(state.SessionParams{ID: string(rune('a' + i))})
	}
